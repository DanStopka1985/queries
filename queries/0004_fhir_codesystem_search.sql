...
CREATE OR REPLACE FUNCTION fhir.fhir_codesystem_search(query json)
  RETURNS json AS
$BODY$
...
begin 
 ...
 __count := coalesce(get_value_of_param(_params, '_count'), (select coalesce(int_value, default_int_value) from fhir.settings where code = 'codesystemSearchCount' limit 1)::text) ::integer;
